...
namespace Exchange {
	struct MEOrder {
        ...
        MEOrder(TickerId ticker_id, ClientId client_id, OrderId client_order_id, OrderId market_order_id, 
            ...
            market_order_id_(market_order_id), side_(side),
            price_(price), qty_(qty), priority_(priority),
    ...
    //Maps each orderId to MEOrder instance
    typedef std::array<MEOrder *, ME_MAX_ORDER_IDS> OrderHashMap;
    //Maps each clientId to OrderHashMap
    ...
    struct MEOrdersAtPrice {
        ...
        MEOrdersAtPrice(Side side, Price price, MEOrder* first_me_order, MEOrdersAtPrice* prev_entry,
            MEOrdersAtPrice* next_entry) : side_(side), price_(price),
            first_me_order_(first_me_order), prev_entry_(prev_entry), next_entry_(next_entry) {}
